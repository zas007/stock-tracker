...
def update_sector_sheet(ss, date_str, all_buy_codes, all_buy_names, current_prices):
    ...
    member_list = list(all_member_codes)
    print(f"  抓取族群成員完整行情（{len(member_list)} 支）...")
    for i, code in enumerate(member_list):
        result = fetch_stock_quote(code, date_str)
        if result:
            close, pct_str, vol = result
        else:
            _, close, vol = current_prices.get(code, (0.0, 0.0, 0))
            pct_str = "N/A"
        # ★ v10：名稱優先順序：CODE_NAME_MAP > 買超榜 > 代號本身
