...
- **快取日期不符時訊息不明確**
  * 舊訊息：`📋 20260609 快取讀取成功`（讀到舊快取但語意像命中，容易誤解）
  * 新邏輯：`load_cache` 改為 `📋 XXXXXX 快取讀取完成`（中性描述）
  * Step 1.5 日期吻合 → `✅ 快取命中（日期吻合），跳過 Step 2/3 API 抓取`
  * Step 1.5 日期不符 → `⚠️ 快取日期不符（20260609 ≠ 20260610），重新抓取` → 繼續跑 Step 2/3
