...
台灣股市三大法人買超/賣超追蹤 v8
新增：
  1. 籌碼集中度（三法人合計買超張數 ÷ 當日成交量 %）
  2. 投信連續買超標記（連續 ≥ 3 天標 ⭐，≥ 5 天標 🔥）
  3. 融資變化（當日融資增減張數、融資餘額趨勢）
  4. 族群聯動工作表（當買超榜出現族群成員，自動列出同族群漲跌幅）
"""
...
# 出貨風險等級門檻
RISK_LOW    = 0.10   # 獲利 < 10%  → 低
RISK_MID    = 0.15   # 獲利 10~15% → 中
...
# 融資增減警示門檻（張）
MARGIN_WARN = 500    # 單日融資增加 ≥ 500 張 → ⚠️ 散戶跟進

# ═══════════════════════════════════════════════
# ★ v8 產業族群對照表
# 格式：{ "族群名稱": ["代號1", "代號2", ...] }
# 當買超榜出現其中任一成員，整族群當日行情都會列出
# 可自行新增族群
# ═══════════════════════════════════════════════
SECTOR_MAP = {
    "探針卡":        ["6217", "6223", "6510", "6515", "7828", "7815"],
    "AI伺服器":      ["2317", "6669", "3231", "4938", "6414"],
    "光通訊":        ["4966", "3491", "6669", "4977", "3015"],
    "HBM/CoWoS封裝": ["2330", "2303", "3711", "5483", "6147"],
    "散熱":          ["6230", "8215", "3017", "1590", "2059"],
    "PCB":           ["2382", "3037", "6269", "8046", "3019"],
    "記憶體模組":    ["3260", "5347", "4256", "3006"],
    "電源供應":      ["6409", "3045", "5483", "4977"],
}
...
def fetch_stock_day(code, date_str):
    """回傳 (avg_price, close_price, volume_lots)
    volume_lots = 成交股數(千股) → 張數（1張=1千股）
    """
...
def enrich_with_prices(groups, date_str):
    ...
    for group in groups:
        for stock in group:
            all_codes[stock["code"]] = (0.0, 0.0, 0)

    total = len(all_codes)
    print(f"  抓取 {total} 支股票價格（每支間隔 0.5 秒）...")

    for i, code in enumerate(all_codes):
        avg, close, vol = fetch_stock_day(code, date_str)
        all_codes[code] = (avg, close, vol)
        print(f"  [{i+1}/{total}] {code}: 均價={avg:.2f}  收盤={close:.2f}  成交={vol:,}張")
        if i < total - 1:
            time.sleep(0.5)

    for group in groups:
        for stock in group:
            avg, close, vol = all_codes.get(stock["code"], (0.0, 0.0, 0))
            stock["avg_price"] = avg
            stock["close"]     = close
            stock["volume"]    = vol  # ★ v7 新增
...
def update_analysis(ss, date_str, current_prices, current_margin):
    ...
    def consecutive_days(entries):
        if not entries: return 0
        entry_dates = sorted(set(e[0] for e in entries), reverse=True)
        if not entry_dates: return 0
        if len(all_dates) < 2: return len(entry_dates)
        count = 0
        idx = all_dates.index(entry_dates[0]) if entry_dates[0] in all_dates else -1
        if idx < 0: return 1
        for ed in entry_dates:
            expected = all_dates[idx - count] if (idx - count) >= 0 else None
            if ed == expected:
                count += 1
            else:
                break
    ...
    def build_row(s, current_prices, current_margin, sell_hist, disp):
        ...
        f_consec = consecutive_days(s["f"])
        t_consec = consecutive_days(s["t"])
        d_consec = consecutive_days(s["d"])
        ...
        _, close, volume = current_prices.get(code, (0.0, 0.0, 0))
    ...
    sorted_stocks = sorted(buy_map.values(),
                           key=lambda x: unique_days(x["f"])+unique_days(x["t"])+unique_days(x["d"]),
                           reverse=True)
...
def update_sector_sheet(ss, date_str, all_buy_codes, all_buy_names, current_prices):
    ...
    - 每次執行覆蓋最新一份
    - 只列出「今日買超榜有成員」的族群
    - 每個族群內所有成員都列出當日漲跌幅、成交量、收盤價、是否在買超榜
    current_prices: {code: (avg, close, volume)} — 今日已抓過的快取，直接複用
    """
    disp = fmt_date(date_str)
    ...
    if not triggered:
        ws.clear()
        ws.update("A1", [[f"資料日期：{disp}"],
                         ["今日買超榜無任何族群成員出現，無需顯示族群聯動。"]])
        print("  ✅ 族群聯動 今日無觸發")
        return

    # 補抓族群內「未在買超榜、也未被抓過價格」的成員
    extra_codes = set()
    for sector, members in SECTOR_MAP.items():
    ...
    extra_price = {}
    if extra_codes:
        print(f"  補抓族群成員行情（{len(extra_codes)} 支）...")
        for i, code in enumerate(extra_codes):
            result = fetch_stock_quote(code, date_str)
            if result:
                close, pct_str, vol = result
                extra_price[code] = (0.0, close, vol, pct_str)
            else:
                extra_price[code] = (0.0, 0.0, 0, "N/A")
            if i < len(extra_codes) - 1:
                time.sleep(0.4)

    # 組裝工作表內容
    hdrs = ["代號", "股票名稱", "收盤價", "漲跌幅%", "成交量(張)", "是否在買超榜", "觸發族群", "備註"]

    rows = [[f"資料日期：{disp}  ｜  族群聯動分析（有法人買超成員的族群）"] + [""]*7,
            [""]*8]
    ...
    for sector, hit_codes in triggered.items():
        ...
        # 族群標題
        trigger_names = [all_buy_names.get(c, c) for c in hit_codes]
        rows.append([f"【{sector}】　觸發成員：{'、'.join(trigger_names)}"] + [""]*7)
        rows.append(hdrs)

        for code in members:
            # 優先用今日已抓快取
            if code in current_prices:
                _, close, vol = current_prices[code]
                # 漲跌幅：current_prices 沒有前收，從 extra_price 補，或標 N/A
                if code in extra_price:
                    pct_str = extra_price[code][3]
                elif code in hit_set:
                    # 在買超榜的股票我們有 close，但漲跌幅要從 fetch_stock_quote 抓
                    # 這裡用延遲補抓（輕量）
                    result = fetch_stock_quote(code, date_str)
                    pct_str = result[1] if result else "N/A"
                    time.sleep(0.3)
                else:
                    pct_str = "N/A"
            elif code in extra_price:
                _, close, vol, pct_str = extra_price[code]
            else:
                close, vol, pct_str = 0.0, 0, "N/A"

            in_buy = "✅ 買超榜" if code in hit_set else ""
            name   = all_buy_names.get(code, "")  # 若在買超榜有名稱
